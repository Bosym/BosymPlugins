	...
	private String getFriendsCombined(List<PAFPlayer> pFriends) {
		...
		for (int i = 0; i < pFriends.size(); i++) {
			...
			if (!pFriends.get(i).isOnline() || pFriends.get(i).getSettingsWorth(3) == 1) {
				...
				additive += getInstance().getMessagesYml().get("Friends.Command.List.ServerColor");
				additive += ProxyServer.getInstance().getPlayer(pFriends.get(i).getUniqueId()).getServer().getInfo().getName();
				additive += getInstance().getMessagesYml().getString("Friends.Command.List.OnlineColor");
				additive += getInstance().getMessagesYml().get("Friends.Command.List.ServerTitle");
